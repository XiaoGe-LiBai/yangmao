 ...
 * 项目名称：番茄畅听
 * 项目更新：2023-08-24
 * 项目抓包：抓api5-sinfonlineb.novelfm.com下的宝箱url#cookie#x-argus#x-ladon#user-agent填入变量
 * 开启抓包，点击宝箱获得金币，点击弹出的看广告，看完一段广告，搜索/luckycat/novel_fm/v1/task/done/excitation_ad
 * 项目抓包：复制/luckycat/novel_fm/v1/task/done/excitation_ad?后面的值，后面的值#cookie#x-argus#x-ladon#user-agent
 * 项目变量：lekebo_fqct_Cookie
 * 项目定时：每30分钟运行一次
 * 定时规则: * 25 0-23 * * *
 * 
 * 版本功能: 签到、宝箱、广告、抽奖、逛街、音乐、吃饭、睡觉、农场，后期会完善待增加的功能
 * 
 * 更新内容: 优化更多得金币细节，增加音乐赚金，增加抽奖接口。
 * 
...
// ================================== 脚本版本参数 =================================== \\
let scriptVersion = "v1.4";
let update_tines = "2023-08-24";
let update_data = "2023-12-12";   //测试时间
let scriptVersionLatest = "v1.4"; //版本对比
let userCookie = ($.isNode() ? process.env.lekebo_fqct_Cookie : $.getdata('lekebo_fqct_Cookie')) || '';
let userList = [];
let userIdx = 0;
...
let host = 'api5-sinfonlineb.novelfm.com';
...
async function start() {
    ...
    for (let user of userList) {
        ...
        await $.wait(1000);
        taskall.push(await user.shop(2 * 1000));
    }
    await Promise.all(taskall);
    DoubleLog(`\n================ 执行听书宝箱抽奖 ================`)
    taskall = [];
    for (let user of userList) {
        ...
        await $.wait(1000);
        taskall.push(await user.open_lottery_reward(2 * 1000));
    }
    await Promise.all(taskall);
    DoubleLog(`\n================ 执行听音乐赚金币 ================`)
    taskall = [];
...
// ============================================执行项目============================================ \\
    ...
    async invitation(timeout = 2000) {
        return new Promise((resolve) => {
            let url = {
                url: `${hostname}/luckycat/novel_fm/v1/task/done/post_invite_code?${this.ck[0]}`,
                headers: {
                    Host: host,
                    'Connection': 'keep-alive',
				    'Content-Type': 'application/json; charset=utf-8',
    ...
    async getMemberScore(timeout = 2000) {
        return new Promise((resolve) => {
            let url = {
                url: `${hostname}/luckycat/novel_fm/v2/task/page?${this.ck[0]}`,
                headers: {
                    Host: host,
                    'Connection': 'keep-alive',
				    'Content-Type': 'application/json; charset=utf-8',
    ...
    async opensign(timeout = 2000) {
        return new Promise((resolve) => {
            let url = {
                url: `${hostname}/luckycat/novel_fm/v2/task/done/sign_in?${this.ck[0]}`,
                headers: {
                    Host: host,
				    'Connection': 'keep-alive',
				    'Content-Type': 'application/json; charset=utf-8',
    ...
    async signin_video(timeout = 2000) {
        return new Promise((resolve) => {
            let url = {
                url: `${hostname}/luckycat/novel_fm/v1/task/done/excitation_ad_signin?${this.ck[0]}`,
                headers: {
                    Host: host,
				    'Connection': 'keep-alive',
				    'Content-Type': 'application/json; charset=utf-8',
                ...
                body: `{"from":"excitation_ad_signin","position":"task_page","reward_stage":0}`,
            }
            $.post(url, async (error, response, data) => {
                try {
                    if (error) {
    ...
    async open_treasure(timeout = 2000) {
        return new Promise((resolve) => {
            let url = {
                url: `${hostname}/luckycat/novel_fm/v2/task/done/treasure_task?${this.ck[0]}`,
                headers: {
                    Host: host,
				    'Connection': 'keep-alive',
				    'Content-Type': 'application/json; charset=utf-8',
    ...
    async treasure_video(timeout = 2000) {
        return new Promise((resolve) => {
            let url = {
                url: `${hostname}/luckycat/novel_fm/v2/task/done/excitation_ad_treasure_box?${this.ck[0]}`,
                headers: {
                    Host: host,
                    'Connection': 'keep-alive',
				    'Content-Type': 'application/json; charset=utf-8',
    ...
    async open_video(timeout = 2000) {
        return new Promise((resolve) => {
            let url = {
                url: `${hostname}/luckycat/novel_fm/v2/task/done/excitation_ad?${this.ck[0]}`,
                headers: {
                    Host: host,
                    'Connection': 'keep-alive',
				    'Content-Type': 'application/json; charset=utf-8',
    ...
    async open_doneeat(timeid,timeout = 2000) {
        return new Promise((resolve) => {
            let url = {
                url: `${hostname}/luckycat/novel_fm/v1/task/done/meal?_request_from=web&${this.ck[0]}`,
                headers: {
                    Host: host,
                    'Connection': 'Keep-Alive',
                    'Content-Type': 'application/json; charset=utf-8',
    ...
    async doneeat_video(timeout = 2000) {
        return new Promise((resolve) => {
            let url = {
                url: `${hostname}/luckycat/novel_fm/v2/task/done/meal_excitation_ad?${this.ck[0]}`,
                headers: {
				    Host: host,
                    'Connection': 'Keep-Alive',
				    'Content-Type': 'application/json; charset=utf-8',
                ...
                body: `{"from":"meal_watching_ad","reward_stage":0}`,
            }
            $.post(url, async (error, response, data) => {
                try {
                    if (error) {
    ...
    async sleepstart(timeout = 2000) {
        return new Promise((resolve) => {
            let url = {
                url: `${hostname}/luckycat/novel_fm/v1/task/sleep/page?_request_from=web&dynamic_settings_version=51&polling_settings_version=0&${this.ck[0]}`,
                headers: {
                    Host: host,
                    'Connection': 'Keep-Alive',
                    'Content-Type': 'application/json; charset=utf-8',
            ...
            $.get(url, async (error, response, data) => {
                try {
                    if (error) {
                        ...
                        let result = JSON.parse(data);
                        if (result.err_no == 0) {
                            let sleepHour = Math.floor(result.data.income_info.sleep_duration / 36) / 100;
    ...
    async sleepStart(timeout = 2000) {
        return new Promise((resolve) => {
            let url = {
                url: `${hostname}/luckycat/novel_fm/v1/task/done/sleep?_request_from=web&${this.ck[0]}`,
                headers: {
                    Host: host,
                    'Connection': 'Keep-Alive',
                    'Content-Type': 'application/json; charset=utf-8',
    ...
    async sleepStop(timeout = 2000) {
        return new Promise((resolve) => {
            let url = {
                url: `${hostname}/luckycat/novel_fm/v2/sleep/stop/?${this.ck[0]}`,
                headers: {
                    Host: host,
                    'Connection': 'Keep-Alive',
                    'Content-Type': 'application/json; charset=utf-8',
    ...
    async sleep_video(timeout = 2000) {
        return new Promise((resolve) => {
            let url = {
                url: `${hostname}/luckycat/novel_fm/v2/task/done/sleep_ad?${this.ck[0]}`,
                headers: {
                    Host: host,
                    'Connection': 'Keep-Alive',
                    'Content-Type': 'application/json; charset=utf-8',
    ...
    async rain(timeout = 2000) {
        return new Promise((resolve) => {
            let url = {
                url: `${hostname}/luckycat/novel_fm/v1/task/redpacket_rain_detail?${this.ck[0]}`,
                headers: {
                    Host: host,
				    'Connection': 'keep-alive',
				    'Content-Type': 'application/json; charset=utf-8',
            ...
            $.get(url, async (error, response, data) => {
                try {
                    if (error) {
                        ...
                        let result = JSON.parse(data);
                        if (result.err_no == 0) {
                            if (times > result.data.ready_time) {
                                await this.open_rain(result.data.strategy_timestamp,2 * 1000);
                            } else {
                                DoubleLog(`\n ❌ 【${this.index}】金币红包: 距离下一次：${$.time('yyyy-MM-dd HH:mm:ss', result.data.ready_time * 1000)} `);
                            }
                        } else {
    ...
    async open_rain(strategytimestamp,timeout = 2000) {
        return new Promise((resolve) => {
            let url = {
                url: `${hostname}/luckycat/novel_fm/v2/task/done/redpacket_rain?${this.ck[0]}`,
                headers: {
				    Host: host,
                    'Connection': 'Keep-Alive',
				    'Content-Type': 'application/json; charset=utf-8',
                ...
                body: `{"task_key":"redpacket_rain","strategy_timestamp":${strategytimestamp},"with_excitation_ad":true}`,
            }
            $.post(url, async (error, response, data) => {
                try {
                    if (error) {
                        ...
                        let result = JSON.parse(data);
                        if (result.err_no == 0) {
                            DoubleLog(`\n ✅ 【${this.index}】红包奖励: 获得奖励 ${result.data.amount} 金币`);
                        } else {
                            DoubleLog(`\n ❌ 【${this.index}】红包奖励: ${result.err_tips}`)
                        }
                    }
                } catch (e) {
                    $.logErr(e, response);
                            ...
                            if(result.data.task_list_v2 && Array.isArray(result.data.task_list_v2)) {
                                for(let i=0; i < result.data.task_list_v2.length; i++) {
                                    ...
                                    if (taskItem.task_id == "21") {
                                        let tasklist = JSON.parse(taskItem.status_extra);
                                        if(taskItem.is_completed == true) {
                                            DoubleLog(`\n ❌ 【${this.index}】逛街赚钱: 今日${taskItem.action_times}次逛街已达上限，请明日再来`)
                                        } else {
                                            if (tasklist.end_time == null) {
                                                await this.shopping_gold(2 * 1000);
                                            } else {
                                                DoubleLog(`\n ❌ 【${this.index}】逛街赚钱: 下次逛街：${$.time('yyyy-MM-dd HH:mm:ss', tasklist.end_time * 1000)}`);
                                            }
                                        }
                                    }
                                }
                    ...
                    if (error) {
                        ...
                        if (result.err_no == 0) {
                            if(result.data.task_list_v2 && Array.isArray(result.data.task_list_v2)) {
                                for(let i=0; i < result.data.task_list_v2.length; i++) {
                                    let taskItem = result.data.task_list_v2[i];
                                    if (taskItem.task_id == "64") {
                                        if(taskItem.is_completed == true) {
                                            DoubleLog(`\n ❌ 【${this.index}】商品赚钱: 今日${taskItem.action_times}次逛街已达上限，请明日再来`)
                                        } else {
                                            await this.shopping_done(2 * 1000);
                                        }
                                    }
                                }
                            } else {
                                DoubleLog(`\n ❌ 【${this.index}】商品赚钱: 没有找到任务相关列表`)
                            }
                        } else {
                            DoubleLog(`\n ❌ 【${this.index}】商品赚钱: ${result.err_tips}`)
                        }
                    }
                } catch (e) {
                    $.logErr(e, response);
    ...
    async shopping_done(timeout = 2000) {
        return new Promise((resolve) => {
            let url = {
                url: `${hostname}/luckycat/novel_fm/v1/task/done/browse_products?${this.ck[0]}`,
                headers: {
                    Host: host,
				    'Content-Type': 'application/json; charset=utf-8',
				    'Cookie': this.ck[1],
                ...
                body: `task_key=shopping_earn_money&task_id=21`,
            }
            $.post(url, async (error, response, data) => {
                try {
                    if (error) {
                        ...
                        if (result.err_no == 0) {
                            ...
                            DoubleLog(`\n ❌ 【${this.index}】逛街奖励: ${result.err_tips}`)
                        }
                    }
                } catch (e) {
                    $.logErr(e, response);
    ...
    async shopping_gold(timeout = 2000) {
        return new Promise((resolve) => {
            let url = {
                url: `${hostname}/luckycat/novel_fm/v1/task/done/shopping_earn_money?${this.ck[0]}`,
                headers: {
                    Host: host,
				    'Content-Type': 'application/json; charset=utf-8',
				    'Cookie': this.ck[1],
    ...
    async music_signin(timeout = 2000) {
        return new Promise((resolve) => {
            let url = {
                url: `${hostname}/luckycat/novel_fm/v1/task/done/music_continue_signin?${this.ck[0]}`,
                headers: {
                    Host: host,
				    'Content-Type': 'application/json; charset=utf-8',
				    'Cookie': this.ck[1],
    ...
    async music_signin_video(timeout = 2000) {
        return new Promise((resolve) => {
            let url = {
                url: `${hostname}/luckycat/novel_fm/v1/task/done/excitation_ad_listen_music?${this.ck[0]}`,
                headers: {
                    Host: host,
				    'Content-Type': 'application/json; charset=utf-8',
				    'Cookie': this.ck[1],
            ...
            $.post(url, async (error, response, data) => {
                try {
                    if (error) {
                        ...
                        let result = JSON.parse(data);
                        if (result.err_no == 0) {
                            DoubleLog(`\n ✅ 【${this.index}】听歌广告: 获得奖励 ${result.data.amount} 金币`);
                            await this.open_music_video(2 * 1000);
                        } else {
                            DoubleLog(`\n ❌ 【${this.index}】听歌广告: ${result.err_tips}`)
                        }
                    }
                } catch (e) {
                    $.logErr(e, response);
    ...
    async open_music(timeout = 2000) {
        return new Promise((resolve) => {
            let url = {
                url: `${hostname}/luckycat/novel_fm/v1/task/done/channel_listen?${this.ck[0]}`,
                headers: {
                    Host: host,
				    'Content-Type': 'application/json; charset=utf-8',
				    'Cookie': this.ck[1],
                ...
                body: `{"task_key":"channel_listen_music"}`,
            }
            $.post(url, async (error, response, data) => {
                try {
                    if (error) {
                        ...
                        let result = JSON.parse(data);
                        if (result.err_no == 0) {
                            DoubleLog(`\n ✅ 【${this.index}】听歌奖励: 获得 ${result.data.amount} 金币,广告预计${result.data.excitation_ad.score_amount}金币`);
                            await this.open_music_video(2 * 1000);
                        } else {
    ...
    async open_music_video(timeout = 2000) {
        return new Promise((resolve) => {
            let url = {
                url: `${hostname}/luckycat/novel_fm/v1/task/done/excitation_ad_channel_listen?${this.ck[0]}`,
                headers: {
                    Host: host,
				    'Content-Type': 'application/json; charset=utf-8',
				    'Cookie': this.ck[1],
                ...
                body: `{"from":"listen_list","position":"","reward_stage":0}`,
            }
            $.post(url, async (error, response, data) => {
                try {
                    if (error) {
    ...
    async lottery(timeout = 2000) {
        return new Promise((resolve) => {
            let url = {
                url: `${hostname}/luckycat/novel_fm/v1/lottery/page?${this.ck[0]}`,
                headers: {
                    Host: host,
				    'Connection': 'Keep-Alive',
				    'Content-Type': 'application/json; charset=utf-8',
    ...
    async open_lottery_signin(timeout = 2000) {
        return new Promise((resolve) => {
            let url = {
                url: `${hostname}/luckycat/novel_fm/v1/lottery/continue_lottery?${this.ck[0]}`,
                headers: {
                    Host: host,
				    'Content-Type': 'application/json; charset=utf-8',
				    'Cookie': this.ck[1],
    ...
    async open_lottery_video(timeout = 2000) {
        return new Promise((resolve) => {
            let url = {
                url: `${hostname}/luckycat/novel_fm/v1/task/done/excitation_ad_continue_lottery?${this.ck[0]}`,
                headers: {
                    Host: host,
				    'Content-Type': 'application/json; charset=utf-8',
				    'Cookie': this.ck[1],
    ...
    async open_lottery_reward(timeout = 2000) {
        return new Promise((resolve) => {
            let url = {
                url: `${hostname}/luckycat/novel_fm/v1/lottery/do_lottery?${this.ck[0]}`,
                headers: {
                    Host: host,
				    'Content-Type': 'application/json; charset=utf-8',
				    'Cookie': this.ck[1],
                                        ...
                                        if (taskItem.is_completed == true) {
                                            ...
                                            for(let i=0; i < extra.length; i++) {
                                                ...
                                                if (taskist.is_completed == false) {
                                                    await this.open_readbook(taskist.task_key,2 * 1000);
                                                    break;
                                                }
                                            }
                                        }
    ...
    async open_readbook(taskkey,timeout = 2000) {
        return new Promise((resolve) => {
            let url = {
                url: `${hostname}/luckycat/novel_fm/v1/task/done/daily_read?${this.ck[0]}`,
                headers: {
                    Host: host,
				    'Connection': 'keep-alive',
				    'Content-Type': 'application/json; charset=utf-8',
    ...
    async readbook_video(timeout = 2000) {
        return new Promise((resolve) => {
            let url = {
                url: `${hostname}/luckycat/novel_fm/v1/task/done/excitation_ad_read?${this.ck[0]}`,
                headers: {
                    Host: host,
				    'Connection': 'keep-alive',
				    'Content-Type': 'application/json; charset=utf-8',
    ...
    async open_listening(taskkey,timeout = 2000) {
        return new Promise((resolve) => {
            let url = {
                url: `${hostname}/luckycat/novel_fm/v1/task/done/${taskkey}?${this.ck[0]}`,
                headers: {
                    Host: host,
				    'Connection': 'keep-alive',
				    'Content-Type': 'application/json; charset=utf-8',
    ...
    async listening_video(timeout = 2000) {
        return new Promise((resolve) => {
            let url = {
                url: `${hostname}/luckycat/novel_fm/v1/task/done/excitation_ad_daily_read?${this.ck[0]}`,
                headers: {
                    Host: host,
				    'Connection': 'keep-alive',
				    'Content-Type': 'application/json; charset=utf-8',
