 ...
 * 
 * 更新内容: 优化看广告速度，增加直播宝箱、增加在指定时间内没有得金币停止看广告功能。时间在下面参数设置
 * 
...
let boxstoptims = "10";       //宝箱广告超指定时间停止 填写注意：1代表1分钟，3代表3分钟
let stoptims = "7";           //看广告超指定时间停止   填写注意：1代表1分钟，3代表3分钟
let appendtims = "5";         //追加广告超指定时间停止 填写注意：1代表1分钟，3代表3分钟
//===============脚本版本=================//
let scriptVersion = "v2.6";
...
// ============================================执行项目============================================ \\
                                ...
                                if (result.data.amount > 19) {
                                    ...
                                    while (!this.videoadstop1) {
                                        ...
                                        if (this.vtstop1 == true) {
                                            DoubleLog(`\n ❌ 【${this.index}】追加奖励: 在您指定${stoptimes}分钟时间内没领到金币停止`);
                                            break;
                                        } else {
                            ...
                            if(result.data.task_list && Array.isArray(result.data.task_list)) {
                                for(let i=0; i < result.data.task_list.length; i++) {
                                    ...
                                    if(taskItem.task_id == "1336") {
                                        ...
                                        if (extra.DailyTreasureFrisbeeInfo.IsCompleted == true) {
                                            DoubleLog(`\n ❌ 【${this.index}】广告赚金: 每天可完成${taskItem.action_times}次, 已完成 ${extra.DailyTreasureFrisbeeInfo.ActionTimes}/${taskItem.action_times} 次`);
                                        } else {
                                            await this.open_live(2 * 1000);
                                        }
                                    }
                                }
                            } else {
    ...
    async open_live(timeout = 2000) {
        return new Promise((resolve) => {
            let url = {
                url: `${hostname}/luckycat/aweme/v1/task/done/live_treasure?${this.ck[0]}`,
                headers: {
                    Host: host,
				    'Content-Type': 'application/json; charset=utf-8',
                    'user-agent': this.ck[4],
            ...
            $.get(url, async (error, response, data) => {
                try {
                    ...
                    if (result == true) {
                        let result = JSON.parse(data);
                        if (result.err_no == 0) {
                            DoubleLog(`\n ✅ 【${this.index}】直播宝箱: 获得奖励 ${result.data.amount} 金币`);
                        } else {
                            DoubleLog(`\n ❌ 【${this.index}】直播宝箱: ${result.err_tips}`)
                        }
                    }
