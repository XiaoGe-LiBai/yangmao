 ...
 * 项目更新：2023-08-06
 * 项目抓包：手机号&密码
 * 项目变量：lekebo_ylw_Cookie
 * 项目定时：每30分钟运行一次
 * 定时规则: * 30 * * * *
 * 
 * 版本功能: 签到、看资讯、阅读、分享、游览、评论，后期会完善待增加的功能
 * 
...
async function start() {
    ...
    taskall = [];
    for (let user of userList) {
        taskall.push(await user.task(1178,"浏览新闻",2 * 1000));
        await $.wait(1000);
    }
    await Promise.all(taskall);
    DoubleLog(`\n================ 执行分享资讯得金 ================`)
    taskall = [];
    for (let user of userList) {
        taskall.push(await user.task(1174,"分享资讯",2 * 1000));
        await $.wait(1000);
    }
    await Promise.all(taskall);
    DoubleLog(`\n================ 执行资讯评论得金 ================`)
    taskall = [];
    for (let user of userList) {
        taskall.push(await user.task(1177,"资讯评论",2 * 1000));
        await $.wait(1000);
    }
    await Promise.all(taskall);
    DoubleLog(`\n================ 执行资讯点赞得金 ================`)
    taskall = [];
    for (let user of userList) {
        taskall.push(await user.task(1176,"资讯点赞",2 * 1000));
        await $.wait(1000);
    }
    await Promise.all(taskall);
}
            ...
            $.get(url, async (error, response, data) => {
                try {
                    if (error) {
                        ...
                        let result = JSON.parse(data);
                        if (result.code == 0) {
                            DoubleLog(`\n ✅ 【${this.index}】浏览任务: 完成获得 ${result.data.score_notify.integral} 积分`);
                        } else {
            ...
            $.post(url, async (error, response, data) => {
                try {
                    if (error) {
                        ...
                        if (result.code == 0) {
                            if (result.data != null) {
                                DoubleLog(`\n ✅ 【${this.index}】分享任务: 完成获得 ${result.data.score_notify.integral} 积分`);
                            } else {
                                DoubleLog(`\n ✅ 【${this.index}】分享任务: ${id} 分享成功`);
                            }
                        } else {
                ...
                try {
                    if (error) {
                        ...
                        if (result.code == 0) {
                            ...
                            await $.wait(1000);
                            DoubleLog(`\n ✅ 【${this.index}】评论任务: 完成获得 ${result.data.score_notify.integral} 积分`);
                        } else {
            ...
            $.post(url, async (error, response, data) => {
                try {
                    if (error) {
                        ...
                        if (result.code == 0) {
                            if (result.data != null) {
                                DoubleLog(`\n ✅ 【${this.index}】点赞任务: 完成获得 ${result.data.score_notify.integral} 积分`);
                            } else {
                                DoubleLog(`\n ✅ 【${this.index}】点赞任务: ${id} 点赞成功`);
                            }
                        } else {
            ...
            $.post(url, async (error, response, data) => {
                try {
                    if (error) {
                        ...
                        if (result.code == 0) {
                            if (result.data != null) {
                                DoubleLog(`\n ✅ 【${this.index}】服务任务: 完成获得 ${result.data.score_notify.integral} 积分`);
                            } else {
                                DoubleLog(`\n ✅ 【${this.index}】服务任务: ${id} 使用成功`);
                            }
                        } else {
