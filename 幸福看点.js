 ...
 * 项目名称：幸福看点
 * 项目更新：2023-08-04
 * 项目抓包：抓xfkd.yichengwangluo.net下的Authorization&device填入变量
 * 项目变量：lekebo_xfkd_Cookie
 * 项目定时：每天运行10次
 * 定时规则: 10 7-22 * * *
 * 
 * 版本功能: 签到、宝箱、气泡、广告、看资讯、阅读、抽奖、闯关、游览、刷视频，后期会完善待增加的功能
 * 邀请下载：https://xfkd.yichengwangluo.net/landing_page/8384223
 * 提现功能: 自己动提现
 * 
...
let userCookie = ($.isNode() ? process.env.lekebo_xfkd_Cookie : $.getdata('lekebo_xfkd_Cookie')) || '';
let userList = [];
let userIdx = 0;
...
let host = 'xfkd.yichengwangluo.net';
...
class UserInfo {
    constructor(str) {
        this.index = ++userIdx;
        this.ck = str.split('&');
		this.authorization = "Bearer" + this.ck[0].replace('Bearer', '');
        this.version = '4';
    }
// ============================================执行项目============================================ \\
                    ...
                    if (error) {
                        ...
                        if (result.code == 0) {
                            if (result.message == "明天再继续") {
                                ...
                                DoubleLog(`\n ✅ 【${this.index}】助力广告: 可获得金额 ${result.result.coin} 金`);
...
function getVersion(timeout = 3 * 1000) {
    return new Promise((resolve) => {
        let url = {
            url: `https://ghproxy.com/https://raw.githubusercontent.com/qq274023/lekebo/master/lekebo_xfkd.js`,
        }
...
function getTimesgin(timestamp) {
    var date = new Date(timestamp);//时间戳为10位需*1000，时间戳为13位的话不需乘1000
    let Y = date.getFullYear(),
        M = (date.getMonth() + 1 < 10 ? '0' + (date.getMonth() + 1) : date.getMonth() + 1),
        D = (date.getDate() < 10 ? '0' + (date.getDate()) : date.getDate()),
