...
class RUN:
    ...
    def login(self, sfurl):
        try:
            ...
            # decoded_url = safe_url_decode(sfurl)
            if "?sign=" not in sfurl and "&sign=" not in sfurl:
                decoded_url = unquote(sfurl)
            else:
                decoded_url = sfurl
...
def is_url_encoded(url):
    """判断URL是否已经编码"""
    try:
        # 尝试解码，如果解码后的字符串和原字符串不同，说明原字符串是编码的
        decoded = unquote(url)
        return decoded != url
    except:
        return False

def safe_url_decode(url):
    """安全地解码URL"""
    try:
        if is_url_encoded(url):
            print(f'🔍 检测到编码URL，正在解码...')
            decoded_url = unquote(url)
            print(f'✅ URL解码成功')
            return decoded_url
        else:
            print(f'🔍 检测到未编码URL，直接使用')
            return url
    except Exception as e:
        print(f'❌ URL处理异常: {str(e)}，使用原URL')
        return url
...
def main():
    ...
    if ENV_NAME in os.environ:
        # print("调用拉菲变量")
        tokens = os.environ[ENV_NAME].split("https")
    ...
    if len(tokens) > 0:
        ...
        for index, infos in enumerate(tokens):
            ...
            run_result = RUN(infos, index).main()
            if not run_result: continue
        if send: send(f'{APP_NAME}通知', send_msg)
