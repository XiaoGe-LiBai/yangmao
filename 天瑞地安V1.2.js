 ...
 * 项目更新：2023-08-14
 * 项目抓包：手机号&密码
 * 项目变量：lekebo_tdra_Cookie
 * 项目定时：每天运行两次
 * 定时规则: 10 7,19 * * *
 * 
 * 版本功能: 签到、阅读、分享、点赞、抽奖，后期会完善待增加的功能
 * 
...
let userCookie = ($.isNode() ? process.env.lekebo_tdra_Cookie : $.getdata('lekebo_tdra_Cookie')) || '';
let userList = [];
let userIdx = 0;
...
let appid = 48;
let salt = "FR*r!isE5W";
let channelId = ["619f0972b40eef3291d28394", "64983fb3e305b412ee03665a", "61a4d501b40eef5eba4db97e", "6498455fe305b412ee036724","63399fbfd86fce04d8498195"]
//---------------------- 自定义变量区域 -----------------------------------
async function start() {
    ...
    taskall = [];
    for (let user of userList) {
        taskall.push(await user.task(1097,"浏览新闻",2 * 1000));
        await $.wait(1000);
    }
    await Promise.all(taskall);
    DoubleLog(`\n================ 执行分享资讯得金 ================`)
    taskall = [];
    for (let user of userList) {
        taskall.push(await user.task(1093,"分享资讯",2 * 1000));
        await $.wait(1000);
    }
    await Promise.all(taskall);
    DoubleLog(`\n================ 执行资讯评论得金 ================`)
    taskall = [];
    for (let user of userList) {
        taskall.push(await user.task(1096,"资讯评论",2 * 1000));
        await $.wait(1000);
    }
    await Promise.all(taskall);
    DoubleLog(`\n================ 执行资讯点赞得金 ================`)
    taskall = [];
    for (let user of userList) {
        taskall.push(await user.task(1095,"资讯点赞",2 * 1000));
        await $.wait(1000);
    }
    await Promise.all(taskall);
    DoubleLog(`\n================ 执行本地服务得金 ================`)
    taskall = [];
    for (let user of userList) {
        taskall.push(await user.task(1092,"本地服务",2 * 1000));
        await $.wait(1000);
    }
    await Promise.all(taskall);
    DoubleLog(`\n================ 执行看新闻兑红包 ================`);
    taskall = [];
    for (let user of userList) {
        taskall.push(await user.current(2 * 1000));
        await $.wait(1000);
        taskall.push(await user.lottery_stop(2 * 1000));
    }
    await Promise.all(taskall);
	// DoubleLog(`\n================ 执行积分兑换物品 ================`);
    // taskall = [];
...
class UserInfo {
    constructor(str) {
        this.index = ++userIdx;
        this.ck = str.split('&');
        this.sessionid = '64d9a84cf34e16592841e976';
        this.requestid = uuid();
        this.xtenantid = '48';
        this.refcode = 'WA74QC';
    }
// ============================================执行项目============================================ \\ 
    ...
    async memberlogin(timeout = 2000) {
        return new Promise((resolve) => {
            let url = {
                ...
                body: `client_id=10004&password=${encodeURIComponent(this.pwd)}&phone_number=${this.ck[0]}`,
            }
            $.post(url, async (error, response, data) => {
                try {
                    if (error) {
        ...
        return new Promise((resolve) => {
            let url = {
                ...
                headers: {
                    ...
                    'User-Agent': `2.31.742;${this.requestid};iPad13,4;IOS;16.2;Appstore`,
                },
                ...
                try {
                    if (error) {
                        ...
                        if (result.code == 0) {
                            ...
                            this.refcode = result.data.account.ref_code;
                            this.nickname = result.data.account.nick_name;
                            this.imageurl = result.data.account.image_url;
        ...
        return new Promise((resolve) => {
            let url = {
                ...
                headers: {
                    ...
                    'user-agent': 'Mozilla/5.0 (Linux; Android 10; Redmi Note 7 Pro Build/QKQ1.190915.002; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/87.0.4280.101 Mobile Safari/537.36;xsb_ruian;xsb_ruian;2.31.742;native_app',
                },
                body: `score=470&account_id=${this.useraccountid}&invitation_number=0&head_img=${this.imageurl}&nick_name=${this.nickname}&mobile=${this.ck[0]}&ref_user_uid=&ref_code=${this.refcode}&username=${this.nickname}&sessionId=${this.usersessionid}`,
            }
            $.post(url, async (error, response, data) => {
                try {
                    if (error) {
                        ...
                        let result = JSON.parse(data);
                        if (result.done == true) {
                            this.usertoken = result.data.access_token
                            await this.open_current(2 * 1000);
                        } else {
                            DoubleLog(`\n ❌ 【${this.index}】答题参数: ${result.message}`)
                        }
                    }
                } catch (e) {
                    $.logErr(e, response);
    ...
    async open_current(timeout = 2000) {
        return new Promise((resolve) => {
            let url = {
            ...
            $.get(url, async (error, response, data) => {
                try {
                    if (error) {
                        ...
                        let result = JSON.parse(data);
                        if (result.data != null) {
                            let num = Number(result.task_total) - Number(result.had_do_total)
                            DoubleLog(`\n ✅ 【${this.index}】检查抽奖: 每天上限 ${result.task_total} 次，当前完成 ${result.had_do_total}/${result.task_total} 次`);
                            await $.wait(1000);
                            if(result.data && Array.isArray(result.data)) {
                                for(let i=0; i < result.data.length; i++) {
                                    let taskItem = result.data[i];
                                    if (taskItem.result_label == "去抽奖") {
                                        if (taskItem.id == 1) {
                                            await this.open_lottery(taskItem.id,2 * 1000);
                                    ...
                                    } else if (taskItem.result_label == "去完成") {
                                        DoubleLog(`\n ❌ 【${this.index}】检查抽奖: ${taskItem.name} 没完成不能抽奖`)
                                    }
                                }
                            }
                        } else {
                            DoubleLog(`\n ❌ 【${this.index}】检查抽奖: ${result.message}`)
                        }
                    }
                } catch (e) {
                    $.logErr(e, response);
    ...
    async open_lottery(lotteryid,timeout = 2000) {
        return new Promise((resolve) => {
            let url = {
                ...
                headers: {
                    ...
                    'authorization': `Bearer ${this.usertoken}`,
                    'User-Agent': `2.31.742;${this.requestid};iPad13,4;IOS;16.2;Appstore`,
                    'referer':'https://app.rabtv.cn/apps/day-redbag/index.html',
                    'content-length':'4',
                },
                body: `id=${lotteryid}`,
            }
            $.post(url, async (error, response, data) => {
                try {
                    let result = JSON.parse(data);
                    if (result.done == true) {
                        DoubleLog(`\n ✅ 【${this.index}】抽奖成功: 获得奖励 ${result.v} 元`);
                    } else {
                        DoubleLog(`\n ❌ 【${this.index}】执行抽奖: ${result.msg}`);
                    }
                } catch (e) {
                    $.logErr(e, response);
                } finally {
    ...
    async lottery_stop(timeout = 2000) {
        return new Promise((resolve) => {
            let url = {
            ...
            $.get(url, async (error, response, data) => {
                try {
                    let result = JSON.parse(data);
                    if (result.data != null) {
                        DoubleLog(`\n ✅ 【${this.index}】抽奖金额: 当前提现余额 ${result.data.money_total} 元`);
                    } else {
                        DoubleLog(`\n ❌ 【${this.index}】抽奖金额: ${result.msg}`);
                    }
                } catch (e) {
                    $.logErr(e, response);
                } finally {
