 ...
 * 项目名称：抖音火山版
 * 项目更新：2023-08-14
 * 项目抓包：抓hotsoon.snssdk.com下的url里面的iid开始抓url#cookie#x-argus#x-ladon填入变量
...
// ============================================执行项目============================================ \\
            ...
            $.get(url, async (error, response, data) => {
                try {
                    if (error) {
                        ...
                        let result = JSON.parse(data);
                        if (result.status_code == 0) {
                            if (result.data.treasure_chest_info.data.last_award_timestamp == null) {
    ...
    async open_treasure(boxtoken,timeout = 2000) {
        return new Promise((resolve) => {
            let url = {
                url: `${hostname}/hotsoon/flame/task_system/task_done/?request_tag_from=lynx&enter_from=video_detail_flame_timer&${this.ck[0]}`,
                headers: {
				    Host: host,
                    'Connection': 'Keep-Alive',
    ...
    async open_treasure_video(boxtoken,timeout = 2000) {
        return new Promise((resolve) => {
            let url = {
                ...
                body: `task_name=treasure_chest_ad&auto_finish=false&token=${boxtoken}`,
            }
            $.post(url, async (error, response, data) => {
                try {
                    if (error) {
    ...
    async open_video(boxtoken,timeout = 2000) {
        return new Promise((resolve) => {
            let url = {
                ...
                body: `task_name=ad&auto_finish=false&token=${boxtoken}`,
            }
            $.post(url, async (error, response, data) => {
                try {
                    if (error) {
