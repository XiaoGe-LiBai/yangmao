...
let update_tines = "2023-08-13";
let update_data = "2023-12-12";   //测试时间
let scriptVersionLatest = "v1.3"; //版本对比
...
// ============================================执行项目============================================ \\
            ...
            $.get(url, async (error, response, data) => {
                try {
                    if (error) {
                        ...
                        let result = JSON.parse(data);
                        if (result.status_code == 0) {
                            //console.log(result.data.daily_task_info_v2.sign_in)
							if(result.data.daily_task_info_v2.sign_in.today_signed == true) {
								DoubleLog(`\n ❌ 【${this.index}】签到信息: 今天已签到，已连续签到 ${result.data.daily_task_info_v2.sign_in.signed_days} 天`);
							} else {
								await this.open_signin(result.data.daily_task_info_v2.sign_in.token,2 * 1000);
                                await $.wait(5000);
                                await this.open_signin_status(2 * 1000);
							}
                        } else {
    ...
    async open_signin(timeout = 2000) {
        return new Promise((resolve) => {
            let url = {
                url: `${hostname}/aweme/ughun/incentive/ad/info?req_from=signin_watch_ad&need_next=false&launch_method=enter_launch&${this.ck[0]}`,
                headers: {
                    Host: host,
				    'Content-Type': 'application/json',
					'x-vc-bdturing-sdk-version': '3.6.2.cn',
            ...
            $.get(url, async (error, response, data) => {
                try {
                    if (error) {
                        ...
                        let result = JSON.parse(data);
                        if (result.status_code == 0) {
                            await this.open_signin_video(result.data.token,2 * 1000);
                        } else {
            ...
            $.post(url, async (error, response, data) => {
                try {
                    if (error) {
                        ...
                        let result = JSON.parse(data);
                        if (result.status_code == 0) {
                            DoubleLog(`\n ✅ 【${this.index}】签到广告: 获得奖励 ${result.data.popup.gold_count} 金币`);
                        } else {
                            DoubleLog(`\n ❌ 【${this.index}】签到广告: ${result.data.message}`)
                        }
                    }
                } catch (e) {
                    $.logErr(e, response);
    ...
    async open_signin_status(timeout = 2000) {
        return new Promise((resolve) => {
            let url = {
                url: `${hostname}/aweme/ughun/incentive/ad/info?req_from=re_signin_ad&need_next=false&launch_method=enter_launch&${this.ck[0]}`,
                headers: {
                    Host: host,
				    'Content-Type': 'application/json',
					'x-vc-bdturing-sdk-version': '3.6.2.cn',
            ...
            $.get(url, async (error, response, data) => {
                try {
                    if (error) {
                        ...
                        let result = JSON.parse(data);
                        if (result.status_code == 0) {
                            await this.signin_video_stop(result.data.token,2 * 1000);
                        } else {
                            DoubleLog(`\n ❌ 【${this.index}】签到追加: ${result.data.message}`)
                        }
                    }
                } catch (e) {
                    $.logErr(e, response);
            ...
            $.post(url, async (error, response, data) => {
                try {
                    if (error) {
                        ...
                        let result = JSON.parse(data);
                        if (result.status_code == 0) {
                            DoubleLog(`\n ✅ 【${this.index}】签到追加: 获得奖励 ${result.data.popup.gold_count} 金币`);
                        } else {
                            DoubleLog(`\n ❌ 【${this.index}】签到追加: ${result.data.message}`)
                        }
                    }
