 ...
 * 项目名称：趣头条
 * 项目更新：2023-08-15
 * 项目抓包：抓api.1sapp.com下的url后面的?token=全部 # X-Tk # user-agent填入变量
 * 项目变量：lekebo_qtt_Cookie
 * 项目定时：每30分钟运行一次
 * 定时规则: 10 7-22 * * *
 * 
 * 版本功能: 签到、广告、刷视频，后期会完善待增加的功能
 * 
...
let userCookie = ($.isNode() ? process.env.lekebo_qtt_Cookie : $.getdata('lekebo_qtt_Cookie')) || '';
let userList = [];
let userIdx = 0;
...
async function start() {
    ...
    for (let user of userList) {
        ...
        await $.wait(1000);
        taskall.push(await user.hoursignin(2 * 1000));
    }
    await Promise.all(taskall);
    DoubleLog(`\n================ 执行幸运转盘签到 ================`)
    taskall = [];
    for (let user of userList) {
        taskall.push(await user.signLucky(2 * 1000));
        await $.wait(1000);
    }
    await Promise.all(taskall);
    DoubleLog(`\n================ 执行时段广告赚金 ================`)
    taskall = [];
    for (let user of userList) {
        taskall.push(await user.playone(2 * 1000));
        await $.wait(1000);
        taskall.push(await user.playtwo(2 * 1000));
        await $.wait(1000);
        taskall.push(await user.playthree(2 * 1000));
        await $.wait(1000);
        taskall.push(await user.playfour(2 * 1000));
    }
    await Promise.all(taskall);
    // DoubleLog(`\n================ 执行雨滴赚金 ================`)
    // taskall = [];
    // for (let user of userList) {
    //     taskall.push(await user.test(2 * 1000));
    //     await $.wait(1000);
    // }
    // await Promise.all(taskall);
}
...
// ============================================执行项目============================================ \\
    async getMemberInfo(timeout = 2000) {
        return new Promise((resolve) => {
            let url = {
                url: `${hostname}/app/user/info/member/v1/get?${this.ck[0]}`,
                headers: {
                    Host: host,
                    'Content-Type': 'application/json; charset=UTF-8',
                    'X-Tk': this.ck[1],
                    'X-Qtt-Exptimestamp': times,
                    'User-Agent': this.ck[2],
                },
            }
            $.get(url, async (error, response, data) => {
                try {
                    if (error) {
                        ...
                        if (result.code == 0) {
                            DoubleLog(`\n ✅ 【${this.index}】用户信息: ${result.data.nickname}，拥有${result.data.newCoinsSystem.allCoins}金币`)
                        } else {
    ...
    async signin(timeout = 2000) {
        return new Promise((resolve) => {
            let url = {
                url: `${hostname}/sign/info?${this.ck[0]}`,
                headers: {
                    Host: host,
                    'Content-Type': 'application/json; charset=UTF-8',
                    'X-Tk': this.ck[1],
            ...
            $.get(url, async (error, response, data) => {
                try {
                    if (error) {
                        ...
                        let result = JSON.parse(data);
                        if (result.code == 0) {
                            if (result.data.signIn.show == 1) {
                                DoubleLog(`\n ❌ 【${this.index}】签到信息: 今天已签到，已连续签到 ${result.data.signIn.today} 天`);
                            } else {
    ...
    async open_signin(timeout = 2000) {
        return new Promise((resolve) => {
            let url = {
                url: `${hostname}/sign/sign?${this.ck[0]}`,
                headers: {
                    Host: host,
                    'Content-Type': 'application/json; charset=UTF-8',
                    'X-Tk': this.ck[1],
    ...
    async hoursignin(timeout = 2000) {
        return new Promise((resolve) => {
            let url = {
                url: `${hostname}/mission/intPointReward?${this.ck[0]}`,
                headers: {
                    Host: host,
                    'Content-Type': 'application/json; charset=UTF-8',
                    'X-Tk': this.ck[1],
            ...
            $.get(url, async (error, response, data) => {
                try {
                    if (error) {
                        ...
                        let result = JSON.parse(data);
                        if (result.code == 0) {
                            DoubleLog(`\n ✅ 【${this.index}】时段广告: 获得奖励 ${result.data.amount} 金币`)
                        } else {
                            DoubleLog(`\n ❌ 【${this.index}】时段广告: ${result.message}`)
                        }
                    }
                } catch (e) {
                    $.logErr(e, response);
    ...
    async signLucky(timeout = 2000) {
        return new Promise((resolve) => {
            let url = {
                url: `https://qtt-turntable.qutoutiao.net/press_trigger?${this.ck[0]}`,
                headers: {
            ...
            $.get(url, async (error, response, data) => {
                try {
                    if (error) {
                        ...
                        let result = JSON.parse(data);
                        if (result.code == 0) {
                            DoubleLog(`\n ✅ 【${this.index}】幸运转盘: 获得 ${result.amount_coin} 金币`);
                        } else if (result.code == 3) {
                            DoubleLog(`\n ❌ 【${this.index}】幸运转盘: 抽奖情况：${result.count}/${result.count_limit} 次`)
                        } else {
                            DoubleLog(`\n ❌ 【${this.index}】幸运转盘: 失败`)
                        }
                    }
    ...
    async playone(timeout = 2000) {
        return new Promise((resolve) => {
            let url = {
                url: `${hostname}/sign/adDone?pos=one&${this.ck[0]}`,
                headers: {
                    Host: host,
                    'Content-Type': 'application/json; charset=UTF-8',
                    'X-Tk': this.ck[1],
    ...
    async playtwo(timeout = 2000) {
        return new Promise((resolve) => {
            let url = {
                url: `${hostname}/sign/adDone?pos=two&${this.ck[0]}`,
                headers: {
                    Host: host,
                    'Content-Type': 'application/json; charset=UTF-8',
                    'X-Tk': this.ck[1],
    ...
    async playthree(timeout = 2000) {
        return new Promise((resolve) => {
            let url = {
                url: `${hostname}/sign/adDone?pos=three&${this.ck[0]}`,
                headers: {
                    Host: host,
                    'Content-Type': 'application/json; charset=UTF-8',
                    'X-Tk': this.ck[1],
    ...
    async playfour(timeout = 2000) {
        return new Promise((resolve) => {
            let url = {
                url: `${hostname}/sign/adDone?pos=four&${this.ck[0]}`,
                headers: {
                    Host: host,
                    'Content-Type': 'application/json; charset=UTF-8',
                    'X-Tk': this.ck[1],
    ...
    async test(timeout = 2000) {
        return new Promise((resolve) => {
            let url = {
                url: `${hostname}/act/incomenotice/show?${this.ck[0]}`,
                headers: {
                    Host: host,
                    'Content-Type': 'application/json; charset=UTF-8',
                    'X-Tk': this.ck[1],
            ...
            $.post(url, async (error, response, data) => {
                try {
                    if (error) {
                        ...
                        if (result.code == 0) {
                            console.log(result)
                            //DoubleLog(`\n ✅ 【${this.index}】雨滴赚金: 获得 ${result.amount_coin} 金币`)
                        } else {
                            DoubleLog(`\n ❌ 【${this.index}】雨滴赚金: ${result.message}`)
                        }
                    }
                } catch (e) {
                    $.logErr(e, response);
