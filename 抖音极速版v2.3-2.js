                    ...
                    if (error) {
                        ...
                        if (result.err_no == 0) {
                            if(result.data.task_list && Array.isArray(result.data.task_list)) {
                                for(let i=0; i < result.data.task_list.length; i++) {
                                    let taskItem = result.data.task_list[i];
                                    if (taskItem.task_id == "1400") {
                                        let extra = JSON.parse(taskItem.status_extra);
                                        for(let i=0; i < extra.game_list.length; i++) {
                                            let gameextra = extra.game_list[i];
                                            if (gameextra.name == "种树赚钱") {
                                                this.gameid = gameextra.game_id;
                                                await this.getHomeInfo(gameextra.game_id,2 * 1000);
                                            }
                                        }
                                    }
                                }
