 ...
 * 项目名称：keep早起打卡小程序
 * 项目更新：1.0
 * 项目抓包：域名www.xiaeke.com下的 openid&user_id 的数值
 * 项目变量：keepck
 * 项目定时：
 * 定时规则: 早起打卡时间段为5:00~8:00 每半个小时一次
 * 
 * github仓库：https://github.com/qq274023/lekebo
 * 
 * 交流Q群：104062430 作者:李华 欢迎前来提交bug
 */
...
let update_tines = "2023-05-25";
let update_data = "2023-05-25";
let scriptVersionLatest = "v1.0"; //版本对比
let userCookie = ($.isNode() ? process.env.keepck : $.getdata('keepck')) || '';
let userList = [];
let userIdx = 0;
let userCount = 0;
let times = Math.round(new Date().getTime() / 1000).toString();
let timestamp = Math.round(new Date().getTime()).toString();
let host = 'www.xiaeke.com';//集合访问网址
let hostname = 'https://' + host;
//---------------------- 自定义变量区域 -----------------------------------
async function start() {
    taskall = [];
    for (let user of userList) {
        taskall.push(await user.get(2 * 1000));
    }
    await Promise.all(taskall);
    DoubleLog(`\n================ 执行账号签到赚钱 ================`)
    taskall = [];
    for (let user of userList) {
        taskall.push(await user.daka(2 * 1000));
        taskall.push(await user.energy(2 * 1000));
    }
    await Promise.all(taskall);
}
...
// ============================================执行项目============================================ \\
    async get(timeout = 2000) {    //get查询信息
        return new Promise((resolve) => {
            let url = {
                url: `${hostname}/benmao/index.php/Home/HeartUser/loginWithOpenId?account_type=6&openid=${this.ck[0]}`,
                headers: {
            ...
            $.get(url, async (error, response, data) => {
                try {
                    if (error) {
                        ...
                        if (result.info == '登陆成功') {
                            DoubleLog(`\n ✅ 【${this.index}】登录成功: ${result.data.nickname} `)
                        } else {
                            DoubleLog(`\n ❌ 【${this.index}】失败信息: ${result.error}`)
                        }
                    }
                } catch (e) {
                    $.logErr(e, response);
    ...
    async daka(timeout = 2000) {    //打卡提交信息
        return new Promise((resolve) => {
            let url = {
                url: `${hostname}/benmao/index.php/Home/MorningNote/addNote?user_id=${this.ck[1]}`,
                headers: {
            ...
            $.get(url, async (error, response, data) => {
                try {
                    if (error) {
                        ...
                        let result = JSON.parse(data);
                        if (result.status > 0) {
                            DoubleLog(`\n  ❌【${this.index}】失败信息:  ${result.info} `)
                        } else {
                            DoubleLog(`\n  ✅【${this.index}】成功信息: ${result.info}`)
                        }
                    }
                } catch (e) {
                    $.logErr(e, response);
    ...
    async energy(timeout = 2000) {    //打卡提交信息
        return new Promise((resolve) => {
            let url = {
                url: `${hostname}/benmao/index.php/Home/MorningTree/gainEnergy?user_id=${this.ck[1]}&energy_type=0&energy=10`,
                headers: {
...
function getVersion(timeout = 3 * 1000) {
    return new Promise((resolve) => {
        let url = {
            url: `https://ghproxy.com/https://raw.githubusercontent.com/qq274023/lekebo/master/lekebo_test.js`,
        }
