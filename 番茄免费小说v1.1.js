 ...
 * 定时规则: * 10 0-23 * * *
 * 
 * 版本功能: 签到、宝箱、广告、吃饭、抽奖，后期会完善待增加的功能
 * 
 * 更新内容: 增加广告追加、增加抽奖。
 * 
...
async function start() {
    ...
    for (let user of userList) {
        ...
        await $.wait(1000);
        taskall.push(await user.lottery_number(2 * 1000));
        await $.wait(1000);
...
// ============================================执行项目============================================ \\
    ...
    async invitation(timeout = 2000) {
        return new Promise((resolve) => {
            let url = {
                ...
                body: `{"in_sp_time":0,"invite_code":"V84387418"}`,
            }
    ...
    async treasure_video_status(timeout = 2000) {
        return new Promise((resolve) => {
            let url = {
                ...
                body: `{"from":"task_list","position":"task_page","task_key":"excitation_ad"}`,
            }
            $.post(url, async (error, response, data) => {
                try {
                    if (error) {
                        ...
                        if (result.err_no == 0) {
                            ...
                            DoubleLog(`\n ❌ 【${this.index}】宝箱追加: ${result.err_tips}`);
                        }
                    }
                } catch (e) {
                    $.logErr(e, response);
    ...
    async doneeat_video(timeout = 2000) {
        return new Promise((resolve) => {
            let url = {
                url: `${hostname}/luckycat/novel/v1/task/done/meal_excitation_ad?${this.ck[0]}`,
                headers: {
				    Host: host,
                    'Connection': 'Keep-Alive',
				    'Content-Type': 'application/json; charset=utf-8',
    ...
    async lottery(timeout = 2000) {
        return new Promise((resolve) => {
            let url = {
                url: `${hostname}/luckycat/novel/v1/lottery/page?${this.ck[0]}`,
                headers: {
                    Host: host,
				    'Connection': 'Keep-Alive',
				    'Content-Type': 'application/json; charset=utf-8',
    ...
    async open_lottery_signin(timeout = 2000) {
        return new Promise((resolve) => {
            let url = {
                url: `${hostname}/luckycat/novel/v1/lottery/continue_lottery?${this.ck[0]}`,
                headers: {
                    Host: host,
				    'Content-Type': 'application/json; charset=utf-8',
				    'Cookie': this.ck[1],
    ...
    async open_lottery_video(timeout = 2000) {
        return new Promise((resolve) => {
            let url = {
                url: `${hostname}/luckycat/novel/v1/task/done/excitation_ad_continue_lottery?${this.ck[0]}`,
                headers: {
                    Host: host,
                    'Connection': 'Keep-Alive',
				    'Content-Type': 'application/json; charset=utf-8',
    ...
    async lottery_number(timeout = 2000) {
        return new Promise((resolve) => {
            let url = {
                url: `${hostname}/luckycat/novel/v1/lottery/lottery_task_page?${this.ck[0]}`,
                headers: {
                    Host: host,
				    'Connection': 'Keep-Alive',
				    'Content-Type': 'application/json; charset=utf-8',
                        ...
                        if (result.err_no == 0) {
                            if(result.data.task_list && Array.isArray(result.data.task_list)) {
                                for(let i=0; i < result.data.task_list.length; i++) {
                                    ...
                                    if (taskItem.task_id == "73") {
                                        if (taskItem.completed == true) {
                                            DoubleLog(`\n ❌ 【${this.index}】视频抽奖: 每天可完成${taskItem.chance_times}次, 已完成 ${taskItem.completed_times}/${taskItem.chance_times} 次`);
                                        } else {
                                            DoubleLog(`\n ✅ 【${this.index}】视频抽奖: 每天可完成${taskItem.chance_times}次, 已完成 ${taskItem.completed_times}/${taskItem.chance_times} 次`);
                                            await this.open_lottery_number(2 * 1000);
                                        }
                                    }
                                }
                            }
                        } else {
                            DoubleLog(`\n ❌ 【${this.index}】视频抽奖: ${result.err_tips}`)
                        }
                    }
                } catch (e) {
                    $.logErr(e, response);
    ...
    async open_lottery_number(timeout = 2000) {
        return new Promise((resolve) => {
            let url = {
                url: `${hostname}/luckycat/novel/v1/lottery/update_chance?${this.ck[0]}`,
                headers: {
                    Host: host,
				    'Content-Type': 'application/json; charset=utf-8',
				    'Cookie': this.ck[1],
            ...
            $.post(url, async (error, response, data) => {
                try {
                    if (error) {
                        ...
                        let result = JSON.parse(data);
                        if (result.err_no == 0) {
                            await this.open_lotteryreward_video(2 * 1000);
                            await $.wait(2000);
                            await this.open_lottery_reward_video(2 * 1000);
                        } else {
                            DoubleLog(`\n ❌ 【${this.index}】增加抽奖: ${result.err_tips}`)
                        }
                    }
                } catch (e) {
                    $.logErr(e, response);
    ...
    async open_lottery_reward_video(timeout = 2000) {
        return new Promise((resolve) => {
            let url = {
                url: `${hostname}/luckycat/novel/v1/lottery/do_lottery?${this.ck[0]}`,
                headers: {
                    Host: host,
				    'Content-Type': 'application/json; charset=utf-8',
				    'Cookie': this.ck[1],
            ...
            $.post(url, async (error, response, data) => {
                try {
                    if (error) {
                        ...
                        let result = JSON.parse(data);
                        if (result.err_no == 0) {
                            DoubleLog(`\n ✅ 【${this.index}】抽奖状态: 获得奖励 ${result.data.reward.amount} 金币`);
                        } else {
                            DoubleLog(`\n ❌ 【${this.index}】抽奖状态: ${result.err_tips}`)
                        }
                    }
                } catch (e) {
                    $.logErr(e, response);
    ...
    async open_lotteryreward_video(timeout = 2000) {
        return new Promise((resolve) => {
            let url = {
                url: `${hostname}/luckycat/novel/v1/task/done/excitation_ad_repeat?${this.ck[0]}`,
                headers: {
                    Host: host,
				    'Content-Type': 'application/json; charset=utf-8',
				    'Cookie': this.ck[1],
                ...
                body: `{"task_key":"excitation_ad_repeat"}`,
            }
            $.post(url, async (error, response, data) => {
                try {
                    if (error) {
                        ...
                        let result = JSON.parse(data);
                        if (result.err_no == 0) {
                            DoubleLog(`\n ✅ 【${this.index}】抽奖广告: 获得奖励 ${result.data.amount} 金币`);
                        } else {
                            DoubleLog(`\n ❌ 【${this.index}】抽奖广告: ${result.err_tips}`)
                        }
                    }
                } catch (e) {
                    $.logErr(e, response);
                    ...
                    if (error) {
                        ...
                        if (result.err_no == 0) {
                            if(result.data.task_list_v2 && Array.isArray(result.data.task_list_v2)) {
                                for(let i=0; i < result.data.task_list_v2.length; i++) {
                                    let taskItem = result.data.task_list_v2[i];
                                    if (taskItem.task_id == "13") {
                                        if (taskItem.completed == true) {
                                            DoubleLog(`\n ❌ 【${this.index}】逛街赚钱: 每天可完成${taskItem.action_times}次, 已完成 ${taskItem.continue_comp_cnt}/${taskItem.action_times} 次`);
                                        } else {
                                            if (taskItem.status_extra != null) {
                                                await this.open_shopping(2 * 1000);
                                            } else {
                                                let tasklist = JSON.parse(taskItem.status_extra);
                                                let next_time = tasklist.end_time;
                                                if (tasklist.end_time != null) {
                                                    DoubleLog(`\n ❌ 【${this.index}】逛街赚钱: 下次广告：${$.time('yyyy-MM-dd HH:mm:ss', next_time * 1000)} 分钟`);
                                                } else {
                                                    DoubleLog(`\n ✅ 【${this.index}】逛街赚钱: 每天可完成${taskItem.action_times}次, 已完成 ${taskItem.continue_comp_cnt}/${taskItem.action_times} 次`);
                                                    //await this.open_shopping(2 * 1000);
                                                }
    ...
    async open_shopping(timeout = 2000) {
        return new Promise((resolve) => {
            let url = {
                url: `${hostname}/luckycat/novel/v1/task/done/shopping_gold?mode=done&${this.ck[0]}`,
                headers: {
                    Host: host,
                    'Connection': 'Keep-Alive',
