...
// ============================================执行项目============================================ \\
            ...
            $.get(url, async (error, response, data) => {
                try {
                    if (error) {
                        ...
                        let result = JSON.parse(data);
                        if (result.err_no == 0) {
                            if(result.data.resource_list && Array.isArray(result.data.resource_list)) {
                                for(let i=0; i < result.data.resource_list.length; i++) {
                                    let taskItem = result.data.resource_list[i];
                                    if (taskItem.card_key == "normal") {
                                        for(let i=0; i < taskItem.resource_data.task_list.length; i++) {
                                            let tasklist = taskItem.resource_data.task_list[i];
                                            if (tasklist.task_id == "1334") {
                                                if(tasklist.action_cycle >= tasklist.action_times) {
                                                    DoubleLog(`\n ❌ 【${this.index}】逛街赚钱: 每日可完成 ${tasklist.action_cycle}/${tasklist.action_times} 次`)
                                                } else {
                                                    DoubleLog(`\n ✅ 【${this.index}】逛街赚钱: 每日可完成 ${tasklist.action_cycle}/${tasklist.action_times} 次`)
                                                    await $.wait(2000);
    ...
    async shopping_ad(timeout = 2000) {
        return new Promise((resolve) => {
            let url = {
            ...
            $.get(url, async (error, response, data) => {
                try {
                    if (error) {
                        ...
                        let result = JSON.parse(data);
                        if (result.code == 0) {
                            DoubleLog(`\n ✅ 【${this.index}】逛街浏览: 正在浏览页面 5 秒后在操作`)
                        } else {
                            DoubleLog(`\n ❌ 【${this.index}】逛街浏览: ${result.msg}`)
                        }
                    }
                } catch (e) {
                    $.logErr(e, response);
